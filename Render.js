...
var PhysicsObject = [];//Daleliu masyvas
var POI = 0;//Physics Object Index

var CubeSideLenght = 100; //Ašies kordinates limitas 


//Sukuria naują dalelę
function PhysicsObjectCreate(Cords,Radius,Mass,Speed,Color)
{
	PhysicsObject[POI] = [];
	PhysicsObject[POI].Cords = Cords;
	PhysicsObject[POI].Radius = Radius;
	PhysicsObject[POI].Mass = Mass;
	PhysicsObject[POI].Speed = Speed;
	PhysicsObject[POI].Color = Color;
...
var GravConst = 1000;//Lygties konstanta
	...
	for(let i = 0; i < POI;i++)
	{
		...
		for(let i2 = 0;i2 < POI;i2++)
		{
			if(i != i2)
			{
				...
				let R2 = PhysicsObject[i2].Radius;
				...
				let Force = -(GravConst*M1*M2)/DistancePow2;
				if(Distance < R1+R2)
				{
					Force*=-1;
				}
...
function RenderFrame()
{
	...
	for(let i = 0; i < POI;i++)
	{
		...
		gl.uniform3f(ColorUniform,PhysicsObject[i].Color[0],
		PhysicsObject[i].Color[1],PhysicsObject[i].Color[2]);
